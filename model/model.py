...
class ThesisTaggingModel_cascade_sn(BaseModel):
    def __init__(self, dim_embeddings, num_classes, embedding, hidden_size=128,
            num_layers=1,  rnn_dropout=0.2, clf_dropout=0.3, bidirectional=False):
        super().__init__()
        self.hidden_size = hidden_size
        ...
        self.clf2_linear1 = nn.Linear(self.hidden_size + self.num_classes + 1, hidden_size // 2)
        self.clf2_linear12 = nn.Linear(hidden_size // 2, 1)
        self.clf2_linear2 = nn.Linear(self.hidden_size + self.num_classes+ 1 + 1, hidden_size // 2)
        self.clf2_linear22 = nn.Linear(hidden_size // 2, 1)
        self.clf2_linear3 = nn.Linear(self.hidden_size + self.num_classes+ 2 + 1, hidden_size // 2)
        self.clf2_linear32 = nn.Linear(hidden_size // 2, 1)
        self.clf2_linear4 = nn.Linear(self.hidden_size + self.num_classes+ 3 + 1, hidden_size // 2)
        self.clf2_linear42 = nn.Linear(hidden_size // 2, 1)
        self.clf2_linear5 = nn.Linear(self.hidden_size + self.num_classes+ 4 + 1, hidden_size // 2)
        self.clf2_linear52 = nn.Linear(hidden_size // 2, 1)
        self.clf2_linear6 = nn.Linear(self.hidden_size + self.num_classes+ 5 + 1, hidden_size // 2)
        self.clf2_linear62 = nn.Linear(hidden_size // 2, 1)
        self.Sigmoid = nn.Sigmoid()
        self.relu = nn.ReLU()

    def submodel(self, former_output,  sentence, number):
        number_tensor = torch.zeros((sentence.size(0), 1), dtype=torch.float32, device="cuda")
        number_tensor = number_tensor.new_full((sentence.size(0), 1), number)
        ...
        x1 = torch.cat((former_output, number_tensor, last_output),1)
        #print("x1",x1.size())
        x = self.clf2_linear1(x1)
        x = self.relu(x)
        x = self.clf2_linear12(x)
    ...
    def forward(self, batch):
        ...
        former_output = init
        for i in range(max_sentence_per_article):
            score = self.submodel( former_output, batch[:, i, :], i)
            output.append(score)
            former_output = score
...
class ThesisTaggingModel_cascade_tf(ThesisTaggingModel_cascade):
    ...
    def forward(self, batch):
        ...
        for i in range(max_sentence_per_article):
            ...
            '''
            That former sentence output similar to final output
            '''
            predict = (score >= 0.5)
            maxclass = torch.argmax(score, dim=1) # make sure every sentence predicted to at least one class
            for i in range(len(predict)):
                predict[i][maxclass[i].item()] = 1
            former_output = predict.type(torch.float32)
...
class ThesisTaggingModel_cascade_bert_5sent(BaseModel):
    def __init__(self, dim_embeddings, num_classes, embedding, hidden_size=768,
            num_layers=1,  rnn_dropout=0.2, clf_dropout=0.3, bidirectional=False):
        super().__init__()
        self.hidden_size = 768
        ...
        self.clf2_linear1 = nn.Linear(self.hidden_size + 4* self.num_classes, hidden_size // 2)
        #self.bn1 = nn.BatchNorm1d(hidden_size // 2)
        self.clf2_linear12 = nn.Linear(hidden_size // 2, 1)
        self.clf2_linear2 = nn.Linear(self.hidden_size + 4* self.num_classes + 1, hidden_size // 2)
        #self.bn2 = nn.BatchNorm1d(hidden_size // 2)
        self.clf2_linear22 = nn.Linear(hidden_size // 2, 1)
        self.clf2_linear3 = nn.Linear(self.hidden_size + 4* self.num_classes + 2, hidden_size // 2)
        #self.bn3 = nn.BatchNorm1d(hidden_size // 2)
        self.clf2_linear32 = nn.Linear(hidden_size // 2, 1)
        self.clf2_linear4 = nn.Linear(self.hidden_size + 4* self.num_classes + 3, hidden_size // 2)
        #self.bn4 = nn.BatchNorm1d(hidden_size // 2)
        self.clf2_linear42 = nn.Linear(hidden_size // 2, 1)
        self.clf2_linear5 = nn.Linear(self.hidden_size + 4* self.num_classes + 4, hidden_size // 2)
        #self.bn5 = nn.BatchNorm1d(hidden_size // 2)
        self.clf2_linear52 = nn.Linear(hidden_size // 2, 1)
        self.clf2_linear6 = nn.Linear(self.hidden_size + 4* self.num_classes + 5, hidden_size // 2)
        #self.bn6 = nn.BatchNorm1d(hidden_size // 2)
        self.clf2_linear62 = nn.Linear(hidden_size // 2, 1)
        self.Sigmoid = nn.Sigmoid()
    ...
    def submodel(self,former_batch2, former_batch, sentence_batch, after_batch, after_batch2):
        former_batch = former_batch.to("cuda")
        after_batch = after_batch.to("cuda")
        
        
        x1 = torch.cat((former_batch2, former_batch, sentence_batch, after_batch, after_batch2),1)
        x = self.clf2_linear1(x1)
        #x = self.bn1(x)
        x = F.relu(x)
        #x = self.dpo(x)
    ...
    def forward(self, batch):
        ...
        for i in range(max_sentence_per_article):
            ...
            score = self.submodel(former_batch2, former_batch, sentence_batch, after_batch, after_batch2)
            for i in range(len(score)):
                output[output_belong[i]].append(score[i][:])
        for i in range(len(output)):
            output[i] = torch.stack(output[i], dim=0)
...
class ThesisTaggingModel_cascade_bert_7sent(BaseModel):
    def __init__(self, dim_embeddings, num_classes, embedding, hidden_size=768,
            num_layers=1,  rnn_dropout=0.2, clf_dropout=0.3, bidirectional=False):
        super().__init__()
        self.hidden_size = 768
        ...
        self.clf2_linear1 = nn.Linear(self.hidden_size + 6* self.num_classes, hidden_size // 2)
        #self.bn1 = nn.BatchNorm1d(hidden_size // 2)
        self.clf2_linear12 = nn.Linear(hidden_size // 2, 1)
        self.clf2_linear2 = nn.Linear(self.hidden_size + 6* self.num_classes + 1, hidden_size // 2)
        #self.bn2 = nn.BatchNorm1d(hidden_size // 2)
        self.clf2_linear22 = nn.Linear(hidden_size // 2, 1)
        self.clf2_linear3 = nn.Linear(self.hidden_size + 6* self.num_classes + 2, hidden_size // 2)
        #self.bn3 = nn.BatchNorm1d(hidden_size // 2)
        self.clf2_linear32 = nn.Linear(hidden_size // 2, 1)
        self.clf2_linear4 = nn.Linear(self.hidden_size + 6* self.num_classes + 3, hidden_size // 2)
        #self.bn4 = nn.BatchNorm1d(hidden_size // 2)
        self.clf2_linear42 = nn.Linear(hidden_size // 2, 1)
        self.clf2_linear5 = nn.Linear(self.hidden_size + 6* self.num_classes + 4, hidden_size // 2)
        #self.bn5 = nn.BatchNorm1d(hidden_size // 2)
        self.clf2_linear52 = nn.Linear(hidden_size // 2, 1)
        self.clf2_linear6 = nn.Linear(self.hidden_size + 6* self.num_classes + 5, hidden_size // 2)
        #self.bn6 = nn.BatchNorm1d(hidden_size // 2)
        self.clf2_linear62 = nn.Linear(hidden_size // 2, 1)
        self.Sigmoid = nn.Sigmoid()
    ...
    def submodel(self,former_batch3, former_batch2, former_batch, sentence_batch, after_batch, after_batch2, after_batch3):
        former_batch = former_batch.to("cuda")
        after_batch = after_batch.to("cuda")
        
        
        x1 = torch.cat((former_batch3, former_batch2, former_batch, sentence_batch, after_batch, after_batch2, after_batch3),1)
        x = self.clf2_linear1(x1)
        #x = self.bn1(x)
        x = F.relu(x)
        #x = self.dpo(x)
    ...
    def forward(self, batch):
        ...
        for i in range(max_sentence_per_article):
            ...
            score = self.submodel(former_batch3, former_batch2, former_batch, sentence_batch, after_batch, after_batch2, after_batch3)
            for i in range(len(score)):
                output[output_belong[i]].append(score[i][:])
        for i in range(len(output)):
            output[i] = torch.stack(output[i], dim=0)
